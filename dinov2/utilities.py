...
from facebookresearch_dinov2_main.dinov2.hub.backbones import dinov2_vits14

# Add DINOv2 directory to Python path
dinov2_path = Path("/home/ubuntu/Anantak/BEVPlace/dinov2/facebookresearch_dinov2_main") 
sys.path.insert(0, str(dinov2_path))


# %%

def load_dinov2_with_custom_weights(weights_path):
    """Using downloaded weights"""
    
    print(f"Creating DINOv2 model with weights from {weights_path}")
    
    # Step 1: Create model architecture (no pretrained weights)
    model = dinov2_vits14(pretrained=False)
    
    # Step 2: Load your custom weights
    try:
        ...
        if missing:
            print(f"  Missing keys: {len(missing)} (usually normal)")
        if unexpected:
            print(f"  Unexpected keys: {len(unexpected)} (usually normal)")
...
class DinoV2ExtractFeatures:
    ...
    def __init__(self, dino_model: _DINO_V2_MODELS, layer: int, 
        ...
        self.weights_path = "checkpoints/dinov2_vits14_pretrain.pth"
        self.dino_model: nn.Module = load_dinov2_with_custom_weights(self.weights_path)
